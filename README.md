...
Kings_Herald is a Node.js process that connects to Discord's gateway using [discord.js](https://discord.js.org/) and a bot token. It responds to two gateway events and runs several scheduled jobs:
...
3. **Weekly recap** — every Sunday at noon Eastern the herald posts in `#general` (falling back to the guild's system channel, then the topmost channel it can post in) celebrating the week's three most-reacted posts (linked, with their authors pinged), the top chatters, the members showered in the most favor, the most time in voice, and the top puzzle solvers, followed by a running points leaderboard of the top 15. Points come from five independent scales: 5/3/1 for the podium posts, 1 point per 10 distinct members who reacted to a member's posts across the week (unique reactors rather than raw reaction counts, and the author's own reactions don't count), 5/3/1 for the most messages sent, 5/3/1 for the most time in voice, and 5/3/1 for the most puzzle points earned that week. Each leaderboard line reads `(+X podium, +Y WoW)`: X is what this recap itself awarded, and Y is the week-over-week change in the member's total (their points now minus a snapshot taken at the end of the previous recap), so it also covers voice time, puzzle solves, duels, and manual adjustments. A short "A Whisper From the Court" aside at the bottom names the pair who spent the most time together in voice; it carries no points. Points persist in DynamoDB (`commands/passive/weeklyRecap.js`, `src/pointsStore.js`).
4. **Daily trivia** — once a day, at a random 15-minute-aligned moment inside the 15-hour window from 9:00 AM Eastern to midnight Eastern, the herald posts a general-knowledge trivia question with four answer buttons (A/B/C/D). Answering is a button click, not a reaction, so choices stay secret — each click gets an ephemeral reply only the clicker sees, and clicking again changes the recorded answer. Thirty minutes later it closes the question: whoever's final answer was correct earns 2 points, and anyone who answered wrong is lightly lambasted by name in the results post. Questions come from a no-repeat cycle over `flavor_text/triviaQuestions.js`, tracked in DynamoDB so a restart doesn't reset it. During Halloween season a round may instead draw from a spooky bank (`flavor_text/halloweenTriviaQuestions.js`) with a reskinned post: a 33% chance per day in September and every day in October. Points persist the same way as the weekly recap (`commands/passive/trivia.js`).
5. **WoW Trivia Wednesdays** — every Wednesday at 9:00 PM Eastern, the herald fires a 5-question free-text WoW trivia session (see `/wow_trivia` above for the rules). Same game as the manual command, just on a fixed weekly schedule instead of an admin triggering it (`commands/passive/wowTrivia.js`).
6. **Voice chat points** — `voiceStateUpdate` events (not a schedule) drive this one: any non-bot member connected to a voice channel (other than the guild's AFK channel) earns points continuously, credited as sessions end or checkpoint rather than at recap time — Discord keeps no historical voice-presence record to scan later the way message history can be. The rate is (members in the channel - 1) points per hour, so time alone in a channel earns nothing, a pair earns 1 per hour, and a call of four earns 3 per hour; it is recalculated whenever someone joins, leaves, or switches channels. Real time in voice also feeds a weekly voice podium (5/3/1), and time shared by each pair of members is tallied for the recap's "Whisper" aside; both reset at the Sunday recap (`commands/passive/voiceTime.js`).
7. **Daily Connections puzzle** — a second daily game, scheduled with the same random 15-minute slot as trivia. The herald posts 24 words (a 3-wide grid) that secretly form six groups of four, and the whole channel plays as one team: a guess is four board words typed separated by commas (case-insensitive), the guild shares 3 wrong guesses, and each member can land only one correct guess per puzzle, so solving it takes several people. A correct guess reveals the category; a guess that is 3-of-4 right gets a gentler hint. Each member who solved a group earns 1 point, but only if the whole puzzle gets solved, and puzzle points also feed the recap's puzzle podium. The puzzle stays open for up to an hour and pings the "Puzzle People" role (`/puzzle_signup`). Game state lives in memory only, so a restart mid-puzzle loses that day's game (`commands/puzzles/connections.js`, puzzle bank in `commands/puzzles/connectionsPuzzles.js`).

`src/index.js` is the entry point. On startup it:

- Reads every `.js` file in `./commands/prompts/` **and** `./commands/passive/preview/`, plus `./commands/puzzles/preview/`, and `require`s each into a `commands` object keyed by filename (`commands/prompts/ping.js` → `/ping`). Passive behaviors in `commands/passive/` and `commands/puzzles/` are wired into client events and schedules directly; `commands/retired/` is not loaded.
...
- **Passive behaviors** (`commands/passive/`) are *push*: nobody types anything — the bot acts on its own in response to a gateway event or a schedule. Five exist today: `celebrate.js` (fires when a message crosses the reaction threshold), `weeklyRecap.js` (the Sunday-noon recap), `trivia.js` (the daily randomized-time trivia round), `wowTrivia.js` (WoW Trivia Wednesdays), and `voiceTime.js` (continuous points for time spent in voice channels). The daily Connections puzzle is the same kind of push behavior but lives in its own `commands/puzzles/` folder (`connections.js`, its puzzle bank, and a hangman word list that no game uses yet), with its own `preview/` folder. These are **not** auto-loaded; each is `require`d and wired to a specific client event or schedule by hand in `src/index.js`.
...
**Preview commands** (`commands/passive/preview/`) bridge the two: a manual `/` trigger that fires a *scheduled* passive behavior on demand, so you can see its output without waiting for the schedule. Each scheduled passive behavior gets one, named to match. Today that's `recap.js` (`/recap` previews `weeklyRecap` in the current channel without awarding points), `trivia.js` (`/trivia [seasonal]` fires a trivia round in the current channel without awarding points, with an optional toggle to force the Halloween bank on or off), `trivia_time.js` (`/trivia_time` reports when today's round is armed to fire), and `wow_trivia.js` (`/wow_trivia` fires a WoW trivia session in the current channel without awarding points), plus two in `commands/puzzles/preview/`: `connections.js` (`/connections` fires a puzzle in the current channel, no points, no role ping) and `puzzle_time.js` (`/puzzle_time` reports when today's puzzle is armed to fire). They dispatch exactly like prompt commands but live beside the behavior they preview and set `adminOnly: true`, which sets `defaultMemberPermissions` on registration so **only members with the Manage Server permission see or can run them** (Discord enforces this — the interaction never reaches the bot for anyone else); `hidden: true` keeps them out of the generated `/help` on top of that. (Event-driven passives like `celebrate` aren't scheduled, so they have no preview.)
...
| `/nobility` | Lists everyone with weekly-recap points, top to bottom, with the noble title their points have earned (ranks run from 5 up to 1900, with the gaps between rungs widening as you climb — see `flavor_text/nobilityRanks.js` for the ladder). |
...
| `/puzzle_signup` | Toggles the "Puzzle People" role on the caller: run once to be summoned when the daily Connections puzzle posts, run again to be removed (`src/puzzleRole.js`). |
...
Six [preview commands](#passive-vs-prompt-commands) exist (`/recap`, `/trivia [seasonal]`, `/trivia_time`, `/wow_trivia [questions]`, `/connections`, and `/puzzle_time`). All are registered with Discord but restricted to members with the Manage Server permission, so they're only visible to admins in the picker. The ones that fire a game (`/recap`, `/trivia`, `/wow_trivia`, `/connections`) default to awarding no points when run manually — only the underlying schedules, firing on their own, pay out for real — while the two `_time` commands just report the armed fire time, visible only to whoever ran them. `/wow_trivia` runs a free-text WoW trivia session (ported from a classic WoW addon's question bank) — `questions` questions in a row (default 1, max 15), each won by whoever types the correct answer in chat first within 30 seconds. Whoever wins the most questions is crowned champion and paid a flat 2 points at the end (ties share the crown, each paid in full, bots excluded from winning); no points change hands per question. Starting a session pings the `/wow_trivia_signup` role, same as the daily trivia pings its own.

A second `adminOnly` + `hidden` command lives directly in `commands/prompts/` rather than beside a passive behavior: `/point_adjust <member> <amount>` grants or deducts a member's ledger points by a set amount (positive to grant, negative to deduct), for correcting mistakes or rewarding things off the books. The visibility restriction works the same way. A third, `/patch_notes [count] [channel]`, announces the most recent commit subjects in the herald's voice; it reads them from `UPDATELOG.md` (regenerate it with `npm run updatelog` first), since the deployed image has no git history to read. Every adjustment — and every `/duel` result — is logged to CloudWatch for audit (see `commands/prompts/point_adjust.js` and `commands/prompts/duel.js`).
...
| Server Members | Member lookups and display names (`/whois`, recap author names, trivia participants). |
...
| Manage Roles | Creating and assigning the "Hear Ye Trivia", "Those Buffed with Arcane Intellect", and "Puzzle People" roles (`/trivia_signup`, `/wow_trivia_signup`, `/puzzle_signup`, `src/triviaRole.js`, `src/wowTriviaRole.js`, `src/puzzleRole.js`). |
...
- **DynamoDB** — a small on-demand table holds the bot's only persistent state, so it survives restarts and redeploys. Several kinds of item share the table, all partitioned by `guildId`: a per-`(guildId, userId)` item holding `displayName`, `points` (the ledger), `duelWins`/`duelLosses`, the voice counters (`totalVoiceSeconds`, `totalWeightedVoiceSeconds`, `weeklyVoiceSeconds`), `weeklyPuzzlePoints`, and `pointsAtLastRecap` (the snapshot behind the recap's WoW figure); one item per resolved `/duel` under a synthetic `DUEL#<timestamp>#...` sort key; one item per member pair under `PAIR#<idA>#<idB>` (weekly shared voice time); and two small no-repeat-cycle items (`TRIVIA#STATE` and `CONNECTIONS#STATE`) under their own synthetic partitions. Synthetic keys can never be mistaken for a real user since Discord IDs are purely numeric. See `src/pointsStore.js`.
