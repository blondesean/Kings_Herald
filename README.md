...
6. **Voice chat points** — `voiceStateUpdate` events (not a schedule) drive this one: any non-bot member connected to a voice channel (other than the guild's AFK channel) earns 2 points per hour, credited continuously as sessions end or checkpoint, rather than at recap time — Discord keeps no historical voice-presence record to scan later the way message history can be. The same time also feeds a weekly voice podium (5/3/1) that's paid out and reset at the Sunday recap alongside the other three scales (`commands/passive/voiceTime.js`).
...
Three [preview commands](#passive-vs-prompt-commands) exist (`/recap` for the weekly recap, `/trivia` for daily trivia, `/wow_trivia [questions]` for WoW Trivia Wednesdays). All three are registered with Discord but restricted to members with the Manage Server permission, so they're only visible to admins in the picker, and all three default to awarding no points when run manually — only the underlying schedules, firing on their own, pay out for real. `/wow_trivia` runs a free-text WoW trivia session (ported from a classic WoW addon's question bank) — `questions` questions in a row (default 1, max 15), each won by whoever types the correct answer in chat first within 30 seconds. Whoever wins the most questions is crowned champion and paid a flat 2 points at the end (ties share the crown, each paid in full, bots excluded from winning); no points change hands per question. Starting a session pings the `/wow_trivia_signup` role, same as the daily trivia pings its own.

A second `adminOnly` + `hidden` command lives directly in `commands/prompts/` rather than beside a passive behavior: `/point_adjust <member> <amount>` grants or deducts a member's ledger points by a set amount (positive to grant, negative to deduct), for correcting mistakes or rewarding things off the books. The visibility restriction works the same way. Every adjustment — and every `/duel` result — is logged to CloudWatch for audit (see `commands/prompts/point_adjust.js` and `commands/prompts/duel.js`).
